...
public class TestListener implements ITestListener {
    ...
    public void onTestFailure(final ITestResult result) {
        try {
             ...
             */
            performFutureAiAnalysis(result);
        } catch (Exception exception) {
     ...
     * @param result test result
     */
    private void attachScreenshot(final ExtentTest test,
                                  final ITestResult result) {
        try {
            final WebDriver driver = DriverManager.getDriver();
            if (driver == null) {
                LOGGER.warn("Driver is null. Screenshot capture skipped for {}.", getTestName(result));return;
            }
            final ScreenshotUtils screenshotUtils =new ScreenshotUtils(driver);
            final String screenshotPath =screenshotUtils.captureScreenshot(getTestName(result));
    ...
    private String getEnvironment() {
        try {
            ...
            LOGGER.warn("Unable to retrieve environment configuration.");
            return UNKNOWN;
        }
    }
    /**
