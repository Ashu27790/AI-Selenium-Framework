package com.ashutosh.ai.framework.test.listener;
...
import com.ashutosh.ai.framework.ai.service.FailureAnalysisService;
...
import com.ashutosh.ai.framework.test.builder.AIRequestBuilder;
...
public class FrameworkTestListener implements ITestListener {
	...
	private static final Logger LOGGER =
			LogManager.getLogger(FrameworkTestListener.class);
	 ...
	 * @param result test result
	 */
	private void performFutureAiAnalysis(
	        final ITestResult result,
	    ...
	    try {
	        ...
	        LOGGER.info("========================================");
	        // Build AI request
	        AIRequest request = AIRequestBuilder.build(result);
	        // Enrich request
	        request.setScreenshotPath(screenshotPath);
	        request.setExecutionTime(executionDuration);
	        // Execute AI analysis
	        FailureAnalysisService service = new FailureAnalysisService();
	        AIResponse response =service.analyzeFailure(request);
	        if (response == null) {
	            LOGGER.warn("AI returned null response.");
	            ExtentTestManager.warning("AI analysis could not be generated.");
	            return;
	        }
	        LOGGER.info("Publishing AI analysis to Extent Report.");
	        // AI Header
	        ExtentTestManager.info("<hr>");
	        ExtentTestManager.info("<h2>🤖 AI FAILURE ANALYSIS</h2>");
	        // Executive Summary
	        ExtentTestManager.logExecutiveSummary( response.getExecutiveSummary());
	        // Severity
	        ExtentTestManager.logSeverity( response.getSeverity());
	        // Failure Category
	        ExtentTestManager.logAiCard("Failure Category",response.getFailureCategory());
	        // Business Impact
	        ExtentTestManager.logAiCard("Business Impact", response.getBusinessImpact());
	        // Root Cause
	        ExtentTestManager.logAiCard( "Root Cause", response.getRootCause());
	        // Possible Reasons
	        ExtentTestManager.logAiCard( "Possible Reasons", response.getPossibleReason());
	        // Suggested Solution
	        ExtentTestManager.logAiCard( "Recommended Solution",response.getSuggestedSolution());
	        // Best Practices
	        ExtentTestManager.logAiCard( "Framework Best Practices", response.getBestPractices());
	        // Retry Recommendation
	        ExtentTestManager.logAiCard( "Retry Recommendation", response.getRetryRecommendation());
	        // Learning Summary
	        ExtentTestManager.logAiCard( "Learning Summary", response.getLearningSummary());
	        // Confidence Score
	        ExtentTestManager.logConfidenceScore( response.getConfidenceScore());
	        // Metrics
	        ExtentTestManager.logAiMetrics( executionDuration, response.getAnalysisTime());
	        // Screenshot
	        ExtentTestManager.attachScreenshot(  screenshotPath);
	        // Raw AI Response
	        ExtentTestManager.logRawResponse( response.getResponse());
	        LOGGER.info( "Enterprise AI Failure Analysis Completed Successfully.");
	    } catch (Exception exception) {
	        LOGGER.error( "Enterprise AI Failure Analysis Failed.", exception);
	        ExtentTestManager.warning( "AI Analysis Failed : " + exception.getMessage());
	    }
	}
